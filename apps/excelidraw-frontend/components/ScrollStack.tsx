...
export const ScrollStackItem: React.FC<ScrollStackItemProps> = ({ children, itemClassName = '', onClick }) => (
  <div
    onClick={onClick}
    className={`scroll-stack-card relative w-full h-80 my-8 p-12 rounded-[40px] shadow-[0_0_30px_rgba(0,0,0,0.1)] box-border origin-top will-change-transform cursor-pointer ${itemClassName}`.trim()}
    style={{
...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  useLayoutEffect(() => {
    ...
    const scrollContainer = useWindowScroll ? window : scrollerRef.current;
    if (scrollContainer) {
